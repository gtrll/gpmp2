...
/* ************************************************************************** */
TEST(mobileBaseUtils, computeArmBasePose) {
  ...
  p2 = Pose2();
  pexp = Pose3();
  pact = computeArmBasePose(p2, base_T_arm, Hact);
  Hexp = numericalDerivative11(boost::function<Pose3(const Pose2&)>(
      boost::bind(&computeArmBasePose, _1, base_T_arm, boost::none)), p2, 1e-6);
...
Pose3 lift_arm_base_pose(const Pose2Vector& posevec, const Pose3& base_T_arm, 
    bool reverse_linact) {
  return liftArmBasePose(posevec.pose(), posevec.configuration()(0), base_T_arm, 
      reverse_linact);
}
...
TEST(mobileBaseUtils, liftBasePose) {
  ...
  arm_pose_act = liftArmBasePose(base_pose2, lift, base_T_arm, reverse_linact, Hact);
