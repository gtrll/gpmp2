...
/* ************************************************************************** */
TEST(VehicleDynamics, simple2DVechileDyanmicsVector3) {
  ...
  Hpexp = numericalDerivative11(boost::function<double(const Vector3&)>(
        boost::bind(simple2DVechileDyanmicsVector3, _1, v, boost::none, boost::none)), p);
  Hvexp = numericalDerivative11(boost::function<double(const Vector3&)>(
        boost::bind(simple2DVechileDyanmicsVector3, p, _1, boost::none, boost::none)), v);
  cact = simple2DVechileDyanmicsVector3(p, v, Hpact, Hvact);
  EXPECT_DOUBLES_EQUAL(cexp, cact, 1e-9);
  EXPECT(assert_equal(Hpexp, Hpact, 1e-6));
  EXPECT(assert_equal(Hvexp, Hvact, 1e-6));
